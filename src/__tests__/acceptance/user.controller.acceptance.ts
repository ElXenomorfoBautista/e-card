...
describe('User Controller', () => {
    ...
    it('gives status 200 when token is passed', async () => {
        const testUser = {
            ...
            permissions: ['CreateAnyUser', 'UpdateAnyUser', 'ViewAnyUser', 'ViewAnyUser'],
        };
    ...
    it('gives status 422 when user details are not correct', async () => {
        const testUser = {
            ...
            username: 'test_user',
            password: 'test_password',
            permissions: ['CreateAnyUser', 'UpdateAnyUser', 'ViewAnyUser', 'DeleteAnyUser'],
        };
        const token = jwt.sign(testUser, process.env.JWT_SECRET as string, {
            expiresIn: 180000,
            issuer: process.env.JWT_ISSUER,
        ...
        await client.post(`/users`).set('authorization', `Bearer ${token}`).send(userToAdd).expect(422);
    });
    ...
    it('gives status 200 and user details when user is added', async () => {
        ...
        expect(response.body).to.have.properties(['username', 'phone', 'firstName']);
    ...
    it('deletes a user successfully', async () => {
        ...
        await client.del(`/users/${reqToAddUser.body.id}`).set('authorization', `Bearer ${token}`).expect(204);
    });
    ...
    async function postUser(token: string) {
        ...
        const reqToAddUser = await client.post(`/users`).set('authorization', `Bearer ${token}`).send(userToAdd);
