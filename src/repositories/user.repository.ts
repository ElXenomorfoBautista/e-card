...
import { DataObject, HasOneRepositoryFactory, repository } from '@loopback/repository';
...
export class UserRepository extends DefaultUserModifyCrudRepository<User, typeof User.prototype.id, UserRelations> {
    ...
    async create(entity: DataObject<User>, options?: Options): Promise<User> {
        ...
        try {
            ...
            const password = await bcrypt.hash(process.env.USER_TEMP_PASSWORD as string, this.saltRounds);
    ...
    async verifyPassword(username: string, password: string): Promise<User> {
        ...
        if (!user || user.deleted || !creds || !creds.password) {
            ...
            throw new HttpErrors.Forbidden(AuthenticateErrorKeys.TempPasswordLoginDisallowed);
        }
        return user;
    }
    ...
    async updatePassword(username: string, password: string, newPassword: string): Promise<User> {
        ...
        if (!user || user.deleted || !creds || !creds.password) {
            ...
            throw new HttpErrors.Unauthorized('Password cannot be same as previous password!');
        }
