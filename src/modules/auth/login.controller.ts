...
import { AuthorizationBindings, authorize, UserPermissionsFn } from 'loopback4-authorization';
...
export class LoginController {
    ...
    async getToken(@requestBody() req: AuthTokenRequest): Promise<TokenResponse> {
        ...
        try {
            const payload: ClientAuthCode<User> = jwt.verify(req.code, authClient.secret, {
    ...
    private async createJWT(payload: ClientAuthCode<User>, authClient: AuthClient): Promise<TokenResponse> {
        try {
